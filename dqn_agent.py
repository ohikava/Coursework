...
from constants import rule_requests, agent_actions, \
                       UNK, REQUEST, MATCH_FOUND, DONE
import random 
import copy 
import numpy as np 
import re 
...
class DQNAgent:
    ...
    def save_weights(self, save_best_model=False):
        ...
        if save_best_model and self.best_model:
            torch.save(self.best_model['beh'], beh_save_file_path)
        else:
        ...
        if save_best_model and self.best_model:
            torch.save(self.best_model['tar'], tar_save_file_path)
        else:
